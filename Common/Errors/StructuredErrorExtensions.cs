...
namespace Il2CppToolkit
{
	public static class StructuredErrorExtensions
	{
		...
		public static void Throw<TError>(this TError errorCode, string message) where TError : Enum
		{
			ErrorSeverity sev = errorCode.GetSeverity();
			if (sev >= ErrorHandler.ErrorThreshhold)
			{
				throw new StructuredException<TError>(errorCode, message);
			}
		}
	}
}