...
namespace Il2CppToolkit.Runtime.Types.corelib.Collections.Generic
{
	 ...
	 * 0x20 - T items[]
	 */
	public class Native__Array<T> : RuntimeObject, IReadOnlyList<T>, INullConstructable
	{
		...
		private void Load()
		{
			...
			ulong readLength = m_specifiedLength ?? Source.ReadValue<ulong>(Address + 0x18);
			if (readLength == 0)
				return;

			ulong typeSize = m_specifiedElementSize ?? Il2CsRuntimeContext.GetTypeSize(typeof(T));
			m_cache = Source.ParentContext.CacheMemory(Address + 0x20, typeSize * readLength);
